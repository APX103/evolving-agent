...
def test_scheduler_basic():
    ...
    try:
        scheduler = AgentScheduler(storage_path=tmpdir)
        ...
        # 6. 持久化验证
        scheduler2 = AgentScheduler(storage_path=tmpdir)
...
async def test_scheduler_async_trigger():
    """测试调度器异步触发"""
...
def test_auto_skill_generator():
    ...
    tmpdir = tempfile.mkdtemp()
    try:
        gen = AutoSkillGenerator(storage_path=tmpdir)
...
def test_session_search_engine():
    """测试 FTS5 会话搜索"""
...
def test_session_search_time_filter():
    """测试时间过滤"""
    print("[E2E] 测试 SessionSearch 时间过滤...")
    from agent.session_search import SessionSearchEngine

    tmpdir = tempfile.mkdtemp()
    db_path = os.path.join(tmpdir, "time_test.db")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_scheduler_async_trigger())
    test_auto_skill_generator()
    test_session_search_engine()
    test_session_search_time_filter()
    print("\n🎉 v4.0 端到端测试全部通过!")
